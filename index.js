...
function channelWithMostContent(channels) {
  return channels.reduce((maxChannel, currentChannel) =>
    totalVideosDuration(currentChannel) > totalVideosDuration(maxChannel)
      ? currentChannel
      : maxChannel
  );
}
...
function longestChannelName(channels) {
  return channels.reduce((longest, current) =>
